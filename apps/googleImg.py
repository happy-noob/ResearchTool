# 运行时需要本机安装 Chrome 浏览器以及Chrome的驱动，同时需要selenium库的支撑
...
search_query = '古陶瓷底款'  # 检索的关键词，可自己输入你想检索的关键字
constraint = ['清康熙','清雍正','清乾隆','宋代']
location_driver = 'utils/chromedriver.exe'  # Chrome驱动程序在电脑中的位置
...
def insert_into_db(data):
    ...
    cursor = db.cursor()
    str_sql = "INSERT INTO TB_OLD(ID,NAME,YEARS,URL) VALUES ('%s','%s','%s','%s')"
    print(str_sql)
    print(data)
    try:
        cursor.execute(str_sql % (data['id'],data['name'],data['years'],data['url']))
        db.commit()
    except():
        db.rollback()
    db.close()
...
class Crawler:
    ...
    def downloadImg(self, driver):
        ...
        picpath = './ImgDownloads/%s' % (foldername)  # 下载到的本地目录
        # 路径不存在时创建一个
        if not os.path.exists(picpath): os.makedirs(picpath)
        # 下载图片的本地路径 /ImageDownload/xxx
